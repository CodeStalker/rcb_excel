...
/*
* Extended by Russell Brown for Textpattern
*/
class Textpattern_Excel_Reader extends Spreadsheet_Excel_Reader {

function dump($row_numbers=false,$col_letters=false,$headers=true,$odd=null,$even=null,$sheet=0,$table_class=null) {

		// pull style info from Excel?
		$s = $this->store_extended_info;

		// to prevent linebreaks when preserving Excel formatting, instead of non-standard <nobr> tags
		$nobr = "white-space:nowrap";

		// only apply table class if user has supplied
		...
		// what is the header?
		$headers = (!$col_letters && $headers==1) ? true : false;
		...
		if ($col_letters) {
			...
			for($i=1;$i<=$this->colcount($sheet);$i++) {
				...
				$style .= $nobr;
				$style = $s ? " style=\"".$style."\"" : "";
				$out .= "\n\t\t<th".$style.">" . strtoupper($this->colindexes[$i]) . "</th>";
			}
			$out .= "</tr></thead>\n";
		}
		...
		for($row=1;$row<=$this->rowcount($sheet);$row++) {
			...
			for($col=1;$col<=$this->colcount($sheet);$col++) {
				...
				if(!$this->sheets[$sheet]['cellsInfo'][$row][$col]['dontprint']) {
					...
					$style .= $nobr;
					$style = $s ? " style=\"".$style."\"" : "";
					$tag = ($headers && $row==1) ? "th" : "td";
					$out .= "\n\t\t<".$tag.$style. ($colspan > 1?" colspan=$colspan":"") . ($rowspan > 1?" rowspan=$rowspan":"") . ">";
					$val = $this->val($row,$col,$sheet);
					if ($val=='') { $val="&nbsp;"; }
					else {
						$val = htmlentities($val);
...
function rcb_excel($atts,$thing) {

		global $rcb_excel,$file_base_path,$production_status;
		...
		if (count($rs) == 0) {
			return rcb_error("File not found in database.  Please check your spelling and ensure that the file has been uploaded from the 'Files' tab in your Textpattern control panel.");
		}

		$filename = $rs["filename"];
		if (strtolower(substr($filename,strlen($filename)-4,4)) != '.xls') {
			return rcb_error("Invalid file format.  File format must be .xls");
		}

		// Check if the file exists
		if (!file_exists($file_base_path.'/'.$filename)) {
			return rcb_error("File not found in directory.  Please re-upload from the 'Files' tab in your Textpattern control panel.");
		}

		// Check if the cache directory exists
		if (!file_exists($file_base_path.'/'.$cachedir)) {
			// Try and make the cache directory
			if(!mkdir($file_base_path.'/'.$cachedir)) {
			return rcb_error("The cache directory could not be created.  Please create it manually.");
			}
		}


		// When was the file last modified?
		$modified = strtotime($rs["modified"]);

		// Generate the cached filename based on last modified date and combination of tag attributes.
		// If any of these variables change, a new cached file will be created - clever, huh?
		$cache_filename = md5($filename.$style.$tableclass.$sheet.$showrownumbers.$showcolletters.$headers.$oddclass.$evenclass.$cachedir.$modified);
		$cache_loc = $file_base_path.'/'.$cachedir.'/'.$cache_filename.'.html';

		// If the cached file already exists, show it and stop the rest of the plugin from executing
		if (file_exists($cache_loc)) {
			$out = file_get_contents($cache_loc);
			return $out;
		}
		// The cached file doesn't exist yet, so make it, save it and show it
		else {
			$rcb_excel = new Textpattern_Excel_Reader($file_base_path.'/'.$filename,$style);
			$out = $rcb_excel->dump($showrownumbers,$showcolletters,$headers,$oddclass,$evenclass,$sheet,$tableclass);
			$fp = fopen($cache_loc,'w');
			fwrite($fp,$out);
			fclose($fp);
			return $out;
		}
...
function rcb_error($err_message) {

	global $production_status;

	// Don't display any errors if site's production status is live
